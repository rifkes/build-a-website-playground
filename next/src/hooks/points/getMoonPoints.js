...
const getMoonPoints = ({ numPoints, }) => {

	const moonPhase = Moon.lunarAgePercent();
	...
	const fullPoints = Math.max(numPoints, 240);
	// latitude vertical
	const numLat = Math.round(Math.sqrt(fullPoints * 3) * 3);
	// longitude horizontal
	const numLon = Math.round(fullPoints * 3);
	...
	for (let i = 0; i < Math.min(numPoints * 3, shuffledPoints.length / 3); i += 3) {
